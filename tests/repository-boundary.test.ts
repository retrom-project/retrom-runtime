...
describe("independent package boundary", () => {
  ...
  it("publishes seven RPG Maker targets plus independent ONS, KiriKiri, Butterscotch, TyranoScript, Java ME and WASM-4 targets", async () => {
    ...
    expect(retromRuntimeProviderDefinition.targets.map((target) => target.id)).toEqual([
      "butterscotch-gamemaker", "fake08", "flash-ruffle", "j2me", "kirikiri2-kag", "msx-webmsx", "onscripter-yuri", "openbor", "play-ps2", "px68k", "rpgmaker-2000", "rpgmaker-2003",
  ...
  it("contains one clean Provider-private adapter role without migration-era aliases", async () => {
    expect(retromRuntimeProviderDefinition.adapters.map((adapter) => adapter.id).sort()).toEqual([
      "butterscotch-web", "easyrpg-web", "fake08-web", "j2me-minijvm-web", "kirikiri2-web", "mkxp-libretro-web", "native-web", "ons-yuri-web", "openbor-web",
    ...
    expect(retromRuntimeProviderDefinition.adapters.map((adapter) => adapter.abi).sort()).toEqual([
      ...
      "native-save", "ons-save", "openbor-host-v1", "play-host-v1", "px68k-host-v1", "ruffle-host-v1", "scummvm-host-v1", "tic80-pmem-v1", "tyranoscript-snapshot-v1", "wasm4-state-v1", "webmsx-host-v1",
    ]);
  ...
  it("declares fixed fork inputs and explicitly marked unreleased candidates", async () => {
    ...
    expect(releaseIds).toEqual(["butterscotch", "easyrpg", "fake08", "j2me", "kirikiri2", "mkxp", "onsyuri", "openbor", "play", "px68k", "ruffle", "scummvm", "tic80", "tyranoscript", "wasm4", "webmsx"]);
