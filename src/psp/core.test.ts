...
it("verifies PSP WASM assets larger than 8 MiB before registering the core", async () => {
  ...
  const files = ["ppsspp.js", "ppsspp.wasm", "ppsspp.data", "ppsspp.worker.mjs", "ppsspp-host.mjs", "ppsspp-io.worker.mjs"];
  ...
  const registration = {abi: "ppsspp-host-v2", createPPSSPPHost: () => {}};
  ...
  const value = await loadPSP({runtimeBaseUrl: "https://core.test/", assetIndex: index,
    game: {kind: "SEEKABLE_BLOB" as const, rangeRequired: true as const, url: "https://game.test/", sizeBytes: 1, sha256: "0".repeat(64)}}, win);
