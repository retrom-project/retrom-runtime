...
describe("RPG Maker checkpoint codec", () => {
  ...
  it("writes the manifest in the Go decoder's RFC 8785 field order", async () => {
    const encoded = await encodeRpgCheckpoint({
      engine: "RPGMV",
      resumeSlot: 21,
      entries: [{ store: "LOCAL_STORAGE", key: "save", mediaType: "application/octet-stream", data: Uint8Array.of(7) }],
    });
    ...
    expect(manifest).toBe([
      ...
      `"sizeBytes":1,"store":"LOCAL_STORAGE"}],"resumeSlot":21,"schemaVersion":1}`,
    ].join(""));
  });
