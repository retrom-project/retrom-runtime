...
DOSBox Pure receives a `FILE_TREE` game resource whose index names one virtual `game.zip` and its exact projected size. Its EmulatorJS adapter opens that entry as a `RANGE_REQUIRED` Content I/O source with a strong ETag pin, probes the first and last bytes, and mounts it into the shared virtual FS. The threaded core reads through the Content I/O worker bridge in bounded 256 KiB pieces. The Content I/O cache fetches demand windows and its bounded prefetch windows; the index and the server's deterministic ZIP overlay do not materialize the source archive in the browser.
