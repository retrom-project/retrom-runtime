...
describe("independent package boundary", () => {
  ...
  it("publishes seven RPG Maker targets plus independent ONS, KiriKiri, Butterscotch, TyranoScript, Java ME and WASM-4 targets", async () => {
    ...
    expect(retromRuntimeProviderDefinition.targets.map((target) => target.id)).toEqual([
      "bbc-jsbeeb", "butterscotch-gamemaker", "fake08", "flash-ruffle", "gbe-pokemini", "j2me", "kirikiri2-kag", "msx-webmsx", "np2kai-pc98", "nxengine", "onscripter-yuri", "openbor", "play-ps2", "ppsspp", "px68k", "rpgmaker-2000", "rpgmaker-2003",
  ...
  it("contains one clean Provider-private adapter role without migration-era aliases", async () => {
    expect(retromRuntimeProviderDefinition.adapters.map((adapter) => adapter.id).sort()).toEqual([
      "butterscotch-web", "easyrpg-web", "fake08-web", "gbe-pokemini-web", "j2me-minijvm-web", "jsbeeb-web", "kirikiri2-web", "mkxp-libretro-web", "native-web", "np2kai-web", "nxengine-web", "ons-yuri-web", "openbor-web",
      "play-web", "ppsspp-web", "px68k-web", "ruffle-web", "samcoupe-web", "scummvm-web", "tic80-web", "tyranoscript-web", "wasm4-web", "webmsx-web",
    ]);
    expect(retromRuntimeProviderDefinition.adapters.map((adapter) => adapter.abi).sort()).toEqual([
      "butterscotch-checkpoint-v2", "easyrpg-save", "fake08-state-v1", "gbe-pokemini-host-v1", "j2me-rms", "jsbeeb-web-v1", "kirikiri-content-io-v1", "mkxp-content-io-v1",
      "native-save", "np2kai-host-v1", "nxengine-host-v1", "ons-save", "openbor-host-v1", "play-host-v2", "ppsspp-host-v3", "px68k-host-v1", "ruffle-host-v1", "samcoupe-web-v1", "scummvm-host-v1", "tic80-pmem-v1", "tyranoscript-snapshot-v1", "wasm4-state-v1", "webmsx-host-v1",
    ]);
