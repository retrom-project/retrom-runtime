...
const filenames = ["UPSTREAM-NOTICE.txt", "webmsx.js"];
export function asWebMSXCandidateSource(release) {
  ...
  if (!validWebMSXSource(source)) {throw invalid();}
  return source;
}

export function validWebMSXSource(source) {
  return exact(source, ["id", "repository", "upstreamCommit", "adapterAbi", "assets"]) &&
    source.id === "webmsx" && source.repository === "https://github.com/retrom-project/WebMSX" &&
    /^[0-9a-f]{40}$/u.test(source.upstreamCommit) && source.adapterAbi === "webmsx-host-v1" &&
...
export async function stageWebMSXCandidate(source, directory, stage) {
...
function output(name) {return name === "UPSTREAM-NOTICE.txt" ? `licenses/webmsx/${name}` : `runtime/webmsx/${name}`;}
function maximum(name) {return name === "webmsx.js" ? 8388608 : 65536;}
...
function invalid() {return new Error("WEBMSX_CANDIDATE_INVALID");}
