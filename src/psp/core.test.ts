...
afterEach(async () => {await Promise.all(owners.splice(0).map(owner => owner.close())); vi.unstubAllGlobals(); document.body.replaceChildren();});
it("verifies PSP WASM assets larger than 8 MiB before registering the core", async () => {
  ...
  vi.stubGlobal("fetch", async (url: string) => {
    fetched.push(url); const response = new Response(url.endsWith(".wasm") ? wasm : small); Object.defineProperty(response, "url", {value: url}); return response;
  });
