type Instance = {
  fileName?: string;
  debug?: boolean;
  Module?: {callMain?: (args: string[]) => unknown; retromWhenAsyncifyDone?: () => Promise<unknown>};
  startGame?: () => unknown;
};
...
export function installAsyncRangeStartup(instance: Instance, disc: Disc) {
  const start = instance.startGame;
  if (!start) {throw new Error("RANGE_STARTUP_UNAVAILABLE");}
  instance.startGame = async () => {
    const module = instance.Module, callMain = module?.callMain;
    if (!module || !callMain || !instance.fileName) {disc.fail(new Error("RANGE_STARTUP_UNAVAILABLE")); return;}
    try {
      callMain.call(module, [...instance.debug ? ["-v"] : [], `/${instance.fileName}`]);
      await module.retromWhenAsyncifyDone?.();
      await disc.idle();
    ...
    } catch (error) {disc.fail(error instanceof Error ? error : new Error("RANGE_STARTUP_FAILED"));}
  };
}
