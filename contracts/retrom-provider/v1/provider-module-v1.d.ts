...
export interface RuntimeGamepadCursorV1 {
  getState(): {enabled: boolean; defaultEnabled: boolean};
  setEnabled(enabled: boolean): void;
}

/** Optional, local-only input observation. No game input is generated by this API. */
export type RuntimeInputObservationV1 = {
  sequence: number;
  atMs: number;
  device: string;
  control: string;
  value: number;
  stage: "BROWSER" | "RUNTIME" | "DELIVERED";
  target: string | null;
  reason: string | null;
  heldMs: number | null;
};
export type RuntimeInputDiagnosticsSnapshotV1 = {
  events: RuntimeInputObservationV1[];
  held: RuntimeInputObservationV1[];
  dropped: number;
  focus: "GAME" | "OTHER" | "UNFOCUSED";
  keyboard: boolean;
  gamepad: boolean;
  delivery: boolean;
  coreRead: false;
};
export interface RuntimeInputDiagnosticsV1 {
  /** Bounded snapshot; must not poll/advance game input or run the core. */
  read(): RuntimeInputDiagnosticsSnapshotV1;
  clear(): void;
  /** Idempotent; removes all observers. Does not release or synthesize game keys. */
  stop(): void;
}
