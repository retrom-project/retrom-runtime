import type {Px68kCore} from "./core.js";
export class Px68kAudio {
  ...
  push(core: Px68kCore) {
    ...
    if (count < 0 || count > 4096 || count % 2 !== 0) {throw new Error("PX68K_AUDIO_INVALID");}
    const offset = core._retrom_audio() / 2;
    if (!Number.isInteger(offset) || offset < 1 || offset + count > core.HEAP16.length) {
      throw new Error("PX68K_AUDIO_INVALID");
    }
