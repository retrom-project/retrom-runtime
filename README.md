...
chooses EasyRPG, mkxp, native Web or another implementation. The Provider validates the frozen Target contract,
resources, private Target options, optional restore, validation and netplay inputs before mounting anything.
...
`client.mjs` digest and Target contract digests. Two builds from identical input must produce identical bytes.
...
Each Target declares the forward-upgrade fields in the Provider declaration:

- `gameCompatibilityLine` is the stable logical game-input contract.
- `checkpoint.writeFormat` is the format written by the active Provider.
- `checkpoint.readFormats` is the exact, non-empty set that can be restored and includes `writeFormat`.

Hosts activate only a higher Provider version. Downgrade, same-version different bytes, deletion of a referenced
Target or an unreadable required checkpoint format fails closed. A checkpoint whose format is absent from the
current Target's `readFormats` remains visible but must not be loaded; hosts do not retain or fall back to an older
Bundle.
