...
const compile = async (name: string) => (await build({entryPoints: [`tests/content-io/${name}.ts`], bundle: true,
...
for (const kind of ["neocd", "scummvm"] as const) {
  test(`[IO-10] BROWSER/${kind}-trace-10000 [BR-08] BROWSER/${kind}-trace-close fixed-seed reads through actual facade and Worker`, async ({page}, testInfo) => {
    test.setTimeout(180000);
    const server = await startFixtureServer({contentModule: await compile("facade-trace"), modules: {"worker.mjs": await compile("trace-worker")}});
    ...
    try {
      ...
      const result = await page.evaluate(async ({kind, identity}) => {
        const url = "/__test__/content.mjs", {facadeTrace} = await import(url) as typeof import("./facade-trace.js");
        return facadeTrace(kind, {identity: identity.identity, sizeBytes: identity.sizeBytes, url: `${location.origin}/objects/cache-trace/trace`,
      ...
      expect(result.snapshots[0].service.networkBytes).toBe(0);
      expect(result.snapshots).toHaveLength(41);
      for (const snapshot of result.snapshots) {
        expect(snapshot.l1 + snapshot.service.lruBytes).toBeLessThanOrEqual(16 * 1024 * 1024);
        expect(snapshot.service.temporaryBytes).toBeLessThanOrEqual(8 * 1024 * 1024);
        expect(snapshot.service.outputBytes).toBeLessThanOrEqual(4 * 1024 * 1024);
        expect(snapshot.service.active).toBeLessThanOrEqual(4);
      }
      if (kind === "neocd") {expect(result.synchronousHits).toBeGreaterThan(0);}
