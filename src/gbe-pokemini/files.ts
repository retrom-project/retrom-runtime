...
export async function fetchFile(source: FileSource, progress: (loaded: number) => void, signal?: AbortSignal) {
  if (!Number.isSafeInteger(source.sizeBytes) || source.sizeBytes < 1 || source.sizeBytes > 64 * 1024 * 1024 ||
    !/^[0-9a-f]{64}$/u.test(source.sha256)) {throw new Error("GBE_FILE_INVALID");}
  let cache: Cache | null = null;
  const url = new URL(source.url, globalThis.location?.href).href;
  try {cache = await caches.open("retrom-gbe-pokemini-content-v1");} catch { /* Storage may be unavailable. */ }
  if (cache) {
    try {
...
async function read(response: Response, source: FileSource, progress: (loaded: number) => void, signal?: AbortSignal) {
  if (!response.ok || !response.body) {throw new Error("GBE_FETCH_FAILED");}
  ...
  try {
    for (;;) {
      ...
      if (done) {break;}
      if (offset + value.length > bytes.length) {throw new Error("GBE_FILE_INVALID");}
    ...
    if (offset !== source.sizeBytes || digest !== source.sha256) {throw new Error("GBE_FILE_INVALID");}
    return bytes;
