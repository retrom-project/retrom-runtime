...
- 核心 adapter 只输出、接收自身未压缩的状态或语义封包；移除 adapter 内已有的传输压缩写入，不得叠加核心 gzip 与公共 gzip。游戏固有存档编码和游戏资源解包不属于传输压缩。
- 公共存储格式通过各 Target 的 `writeFormat`/`readFormats` 显式版本化。旧未压缩、PSP gzip、mkxp compact 存档按原格式分派到兼容读取路径；旧压缩编码只读，不继续写入，也不得按魔数猜测旧格式。
