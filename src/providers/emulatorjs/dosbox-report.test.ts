...
describe("DOSBox Pure's pinned EmulatorJS report", () => {
  it("accepts a verified immutable core release report", async () => {
    const sha = "a".repeat(64);
    const metadata = {schemaVersion: 1, repository: "https://github.com/retrom-project/dosbox-pure",
      tag: "retrom-core-g3a5222c97456-r1", commit: "b".repeat(40), adapterAbi: "emulatorjs-content-io-v1",
      assets: [{filename: "dosbox_pure-thread-wasm.data", observedSha256: sha, sizeBytes: 123}]};
    ...
    try {
      await expect(instance.downloadFile("cores/reports/dosbox_pure.json?v=1", "Reports"))
        .resolves.toEqual({data: {...metadata, buildStart: sha}, headers: {}});
      original.mockResolvedValueOnce({data: {files: [{bytes: new TextEncoder().encode(JSON.stringify({
        ...metadata, assets: [{...metadata.assets[0], observedSha256: "c".repeat(64)}],
      }))}]}, headers: {}});
      await expect(instance.downloadFile("cores/reports/dosbox_pure.json?v=2", "Reports"))
        .rejects.toThrow("PLAYER_DOS_REPORT_INVALID");
    } finally {cleanup();}
  });
