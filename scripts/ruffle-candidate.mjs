...
export function validRuffleSource(source) {
  return exact(source, ["id", "repository", "upstreamCommit", "adapterAbi", "assets"]) &&
    ...
    /^[0-9a-f]{40}$/u.test(source.upstreamCommit) && source.adapterAbi === "ruffle-host-v1" &&
...
export async function stageRuffleCandidate(source, directory, stage) {
...
function output(name) {return name.startsWith("LICENSE") ? `licenses/ruffle/${name}` : `runtime/ruffle/${name}`;}
function maximum(name) {return name === "ruffle.wasm" ? 67108864 : 4194304;}
...
function invalid() {return new Error("RUFFLE_CANDIDATE_INVALID");}
