...
function stableError(error: unknown) {
  ...
  if (error instanceof Error && /^(?:RUNTIME|CHECKPOINT|PLAYER|PROVIDER|RPG|ONS|KIRIKIRI|BUTTERSCOTCH|TYRANOSCRIPT|WASM4|J2ME|SCUMMVM|FANTASY|WEBMSX)_[A-Z0-9_]+$/u.test(error.message)) {
    return error;
  }
